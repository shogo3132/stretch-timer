...
';
  ...
  function makeTaskRow(task,completed){
    ...
    row.append(handle,check,main,more,del,dup);if(window.StretchUI&&StretchUI.bindSwipe)StretchUI.bindSwipe(row,{left:'.task-swipe-delete',right:'.task-swipe-duplicate'});else wireTaskSwipe(row);if(!completed)wireTaskReorder(row,task.id);return row;
  }
