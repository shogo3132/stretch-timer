...
';
  ...
  function bindTaskDetail(){
    ...
    document.getElementById('taskDetailSave').onclick=function(){var task=taskById(detailTaskId);if(!task)return;task.title=(document.getElementById('taskDetailName').value||'').trim()||task.title;task.repeatDaily=detailDaily;task.dueDay=detailDueDay;task.dueTime=detailDueTime;save();renderTaskDetail()};document.getElementById('taskDetailUnschedule').onclick=function(){var api=window.__stretchTimerDailyScheduleV113;if(api&&api.removeTaskFromSelectedDay&&api.removeTaskFromSelectedDay(detailTaskId))renderTasks()};document.getElementById('taskDetailDelete').onclick=function(){var task=taskById(detailTaskId);if(task&&confirm('「'+task.title+'」を削除しますか？'))removeTask(task.id)};document.getElementById('taskCalPrev').onclick=function(){changeTaskMonth(-1)};document.getElementById('taskCalNext').onclick=function(){changeTaskMonth(1)};
  ...
  function renderTaskDetail(){var task=taskById(detailTaskId);if(!task){renderTasks();return}var name=document.getElementById('taskDetailName');name.value=task.title;resizeTaskTextarea(name);var api=window.__stretchTimerDailyScheduleV113,unschedule=document.getElementById('taskDetailUnschedule');if(unschedule)unschedule.hidden=!(api&&api.countTaskOnSelectedDay&&api.countTaskOnSelectedDay(task.id));updateTaskDetailOptions();renderTaskCalendar()}
  ...
  function removeTask(id){state.tasks=state.tasks.filter(function(x){return x.id!==id});var days=state.taskSchedule&&state.taskSchedule.days;if(days)Object.keys(days).forEach(function(key){var day=days[key];if(day&&Array.isArray(day.entries))day.entries=day.entries.filter(function(entry){return entry.taskId!==id})});save();if(typeof currentScreen!=='undefined'&&currentScreen==='taskDetail')renderTasks();else renderTaskLists()}
