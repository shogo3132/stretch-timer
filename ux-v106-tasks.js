...
.task-notify-options{display:flex;align-items:center;flex-wrap:wrap;gap:9px;padding:10px 0 1px;color:#53606a;font-size:13px;font-weight:700}\
.task-notify-options label{display:flex;align-items:center;gap:7px;cursor:pointer}\
.task-notify-options input[type="checkbox"]{width:18px;height:18px;accent-color:#27ae8b}\
.task-notify-minutes{display:flex;align-items:center;gap:5px;color:#69747d;font-size:12px}.task-notify-minutes input{width:52px;height:31px;border:0;border-radius:9px;background:#f1f3f5;color:#273039;text-align:center;font:inherit;font-weight:800;outline:none}\
.task-detail-save{width:100%;min-height:46px}\
.task-detail-unschedule{width:100%;min-height:42px;border:0;border-radius:13px;background:#eef3f2;color:#48615a;font-weight:750;cursor:pointer}\
.task-detail-delete{width:100%;min-height:42px;border:0;border-radius:13px;background:#fff0f1;color:#ba4653;font-weight:800;cursor:pointer}\
...
';
  ...
  function cleanTask(x){
    ...
    return {id:x.id||uid(),title:String(x.title||'').trim()||'名称未設定',repeatDaily:!!x.repeatDaily,dueDay:validDay(x.dueDay),dueTime:validTime(x.dueTime),notifyBeforeEnabled:!!x.notifyBeforeEnabled,notifyBeforeMinutes:Math.max(1,Math.min(180,Math.round(+x.notifyBeforeMinutes||10))),createdDay:String(x.createdDay||''),carriedFrom:String(x.carriedFrom||''),completedAt:Math.max(0,+x.completedAt||0),completedDay:String(x.completedDay||'')};
  }
  ...
  function ensureScreens(){
    ...
    if(!document.getElementById('taskDetail')){
      var detail=document.createElement('main');detail.id='taskDetail';detail.className='screen';detail.innerHTML='<div class="task-detail-wrap"><div class="card task-detail-edit"><textarea id="taskDetailName" class="task-detail-name" maxlength="200" rows="1" aria-label="タスク名"></textarea><div class="task-detail-options"><button id="taskDetailDaily" class="task-daily-chip" type="button">↻ 毎日</button><div class="task-due-wrap"><button id="taskDetailDue" class="task-due-chip" type="button">期限なし</button><input id="taskDetailDueInput" class="task-due-input" type="date"><button id="taskDetailTime" class="task-time-chip" type="button" hidden>時間なし</button><button id="taskDetailClear" class="task-due-clear" type="button" aria-label="期限を外す" hidden>×</button></div></div><div class="task-notify-options"><label><input id="taskDetailNotifyBefore" type="checkbox">開始前にも通知する</label><span id="taskDetailNotifyMinutesWrap" class="task-notify-minutes" hidden><input id="taskDetailNotifyMinutes" type="number" min="1" max="180" inputmode="numeric">分前</span></div><button id="taskDetailSave" class="btn task-detail-save" type="button">保存</button><button id="taskDetailUnschedule" class="task-detail-unschedule" type="button" hidden>この日のタイムテーブルから外す</button><button id="taskDetailDelete" class="task-detail-delete" type="button">このタスクを削除</button></div><div class="task-calendar-card"><div class="task-cal-head"><button id="taskCalPrev" class="task-cal-nav" type="button">‹</button><div id="taskCalTitle" class="task-cal-title"></div><button id="taskCalNext" class="task-cal-nav" type="button">›</button></div><div class="task-cal-week"><div>日</div><div>月</div><div>火</div><div>水</div><div>木</div><div>金</div><div>土</div></div><div id="taskCalDays" class="task-cal-days"></div></div><div class="task-cal-legend"><span class="task-cal-key">今日</span><span class="task-cal-key deadline">締切日</span></div></div>';app.insertBefore(detail,document.getElementById('timer'));
      bindTaskDetail();
    }
  ...
  function bindTaskDetail(){
    ...
    var notify=document.getElementById('taskDetailNotifyBefore'),minutes=document.getElementById('taskDetailNotifyMinutes'),minutesWrap=document.getElementById('taskDetailNotifyMinutesWrap');notify.onchange=function(){minutesWrap.hidden=!notify.checked};document.getElementById('taskDetailSave').onclick=function(){var task=taskById(detailTaskId);if(!task)return;task.title=(document.getElementById('taskDetailName').value||'').trim()||task.title;task.repeatDaily=detailDaily;task.dueDay=detailDueDay;task.dueTime=detailDueTime;task.notifyBeforeEnabled=!!notify.checked;task.notifyBeforeMinutes=Math.max(1,Math.min(180,Math.round(+minutes.value||10)));save();if(window.__stretchTimerDailyScheduleV113&&window.__stretchTimerDailyScheduleV113.syncNotifications)window.__stretchTimerDailyScheduleV113.syncNotifications(true);renderTaskDetail()};document.getElementById('taskDetailUnschedule').onclick=function(){var api=window.__stretchTimerDailyScheduleV113;if(api&&api.removeTaskFromSelectedDay&&api.removeTaskFromSelectedDay(detailTaskId))renderTasks()};document.getElementById('taskDetailDelete').onclick=function(){var task=taskById(detailTaskId);if(task&&confirm('「'+task.title+'」を削除しますか？'))removeTask(task.id)};document.getElementById('taskCalPrev').onclick=function(){changeTaskMonth(-1)};document.getElementById('taskCalNext').onclick=function(){changeTaskMonth(1)};
  ...
  function renderTaskDetail(){var task=taskById(detailTaskId);if(!task){renderTasks();return}var name=document.getElementById('taskDetailName');name.value=task.title;resizeTaskTextarea(name);var notify=document.getElementById('taskDetailNotifyBefore'),minutes=document.getElementById('taskDetailNotifyMinutes'),minutesWrap=document.getElementById('taskDetailNotifyMinutesWrap');if(notify){notify.checked=detailNotifyBefore;minutes.value=detailNotifyMinutes;minutesWrap.hidden=!detailNotifyBefore}var api=window.__stretchTimerDailyScheduleV113,unschedule=document.getElementById('taskDetailUnschedule');if(unschedule)unschedule.hidden=!(api&&api.countTaskOnSelectedDay&&api.countTaskOnSelectedDay(task.id));updateTaskDetailOptions();renderTaskCalendar()}
  function openTaskDetail(id){var task=taskById(id);if(!task)return;detailTaskId=id;detailDueDay=task.dueDay;detailDueTime=task.dueTime;detailDaily=task.repeatDaily;detailNotifyBefore=!!task.notifyBeforeEnabled;detailNotifyMinutes=Math.max(1,Math.min(180,Math.round(+task.notifyBeforeMinutes||10)));if(detailDueDay){var p=detailDueDay.split('-');detailViewDate=new Date(+p[0],+p[1]-1,1)}else detailViewDate=new Date();show('taskDetail','タスク詳細');renderTaskDetail();window.scrollTo(0,0)}
  ...
  function removeTask(id){state.tasks=state.tasks.filter(function(x){return x.id!==id});var days=state.taskSchedule&&state.taskSchedule.days;if(days)Object.keys(days).forEach(function(key){var day=days[key];if(day&&Array.isArray(day.entries))day.entries=day.entries.filter(function(entry){return entry.taskId!==id})});save();if(window.__stretchTimerDailyScheduleV113&&window.__stretchTimerDailyScheduleV113.syncNotifications)window.__stretchTimerDailyScheduleV113.syncNotifications(true);if(typeof currentScreen!=='undefined'&&currentScreen==='taskDetail')renderTasks();else renderTaskLists()}
