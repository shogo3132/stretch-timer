...
';
  ...
  function renderSchedule(){
    ...
    empty.textContent=entries.length?'「今日使うタスク」をタップして開始・終了時間を設定してください':'下のタスク左端のハンドルを長押しして追加できます';empty.style.display=scheduled.length?'none':'block';document.getElementById('dailyTimelineScroll').style.display=scheduled.length?'block':'none';renderNow(range);renderPool(entries);decorateRows(entries);
  }
  ...
  function wireChipReturn(chip,entry){
    ...
    function finish(e){if(e&&e.cancelable)e.preventDefault();var shouldReturn=active&&outside();if(active)logSchedule(shouldReturn?'return-drag-drop':'return-drag-cancel',{entryId:entry.id,taskId:entry.taskId,y:Math.round(currentY)});clear();if(shouldReturn){removeEntry(entry.id);confirmPool('通常のタスクリストへ戻しました')}}
