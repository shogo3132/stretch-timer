(function(){
  ...
  var SNAP=15,HOUR_PX=56,DEFAULT_START=9*60,DEFAULT_DURATION=60,TASK_HOLD_MS=200;
  var COLORS=[
...
.daily-chip{flex:0 0 auto;max-width:190px;min-height:34px;border:1px solid var(--event-line);border-radius:11px;background:var(--event-bg);color:var(--event-text);padding:6px 9px;text-align:left;touch-action:pan-x;-webkit-touch-callout:none}\
.daily-chip.return-holding{transform:scale(.96);filter:saturate(.75);box-shadow:0 0 0 3px rgba(39,174,139,.2)}\
...
';
  ...
  function ensureUi(){
    ...
    if(!root){root=document.createElement('section');root.id='dailySchedule';root.className='daily-schedule';root.innerHTML='<div class="card daily-schedule-card"><div class="daily-schedule-head"><div class="daily-schedule-title">今日のスケジュール</div><div id="dailyScheduleDate" class="daily-schedule-date"></div></div><div id="dailyTimelineScroll" class="daily-timeline-scroll"><div id="dailyTimeline" class="daily-timeline"><div id="dailyEvents" class="daily-events"></div><div id="dailyNow" class="daily-now" hidden></div></div></div><div id="dailyTimelineEmpty" class="daily-schedule-empty">下のタスク左端のハンドルを長押しして追加できます</div></div><div id="dailyPool" class="daily-pool"><div class="daily-pool-head"><span>今日使うタスク</span><span class="daily-pool-hint">左のハンドルを長押し</span></div><div id="dailyPoolList" class="daily-pool-list"></div></div>';page.insertBefore(root,page.firstChild);wirePool(root.querySelector('#dailyPool'))}
    return root;
  }
  ...
  function renderPool(entries){var list=document.getElementById('dailyPoolList');if(!list)return;list.innerHTML='';if(!entries.length){list.innerHTML='<div class="daily-pool-empty">左のハンドルを長押しして追加</div>';return}entries.forEach(function(entry){var task=taskById(entry.taskId);if(!task)return;var chip=document.createElement('button');chip.type='button';chip.className='daily-chip';chip.dataset.entryId=entry.id;chip.title='長押しで今日の予定から外す';applyPalette(chip,entry);chip.innerHTML='<span class="daily-chip-title"></span><span class="daily-chip-time"></span>';chip.querySelector('.daily-chip-title').textContent=task.title;chip.querySelector('.daily-chip-time').textContent=entry.start==null?'時間未設定・長押しで戻す':timeText(entry.start)+'〜'+timeText(entry.end)+'・長押しで戻す';wireChipReturn(chip,entry);list.appendChild(chip)})}
  function wireChipReturn(chip,entry){
    var timer=null,startX=0,startY=0,suppress=false;
    function clear(){clearTimeout(timer);timer=null;chip.classList.remove('return-holding')}
    function begin(x,y,e){clear();startX=x;startY=y;suppress=false;chip.classList.add('return-holding');timer=setTimeout(function(){timer=null;suppress=true;if(window.__stretchDiagnostics&&window.__stretchDiagnostics.log)window.__stretchDiagnostics.log('task-schedule',{status:'return-from-pool',entryId:entry.id,taskId:entry.taskId});if(navigator.vibrate)navigator.vibrate(25);removeEntry(entry.id)},420);if(e&&e.pointerType==='mouse')e.preventDefault()}
    chip.addEventListener('touchstart',function(e){if(e.touches.length!==1)return;var t=e.touches[0];begin(t.clientX,t.clientY,e)},{passive:true});
    chip.addEventListener('touchmove',function(e){if(!timer||e.touches.length!==1)return;var t=e.touches[0];if(Math.hypot(t.clientX-startX,t.clientY-startY)>12)clear()},{passive:true});
    chip.addEventListener('touchend',clear,{passive:true});chip.addEventListener('touchcancel',clear,{passive:true});
    chip.addEventListener('pointerdown',function(e){if(e.pointerType==='touch'||(e.button!=null&&e.button!==0))return;begin(e.clientX,e.clientY,e)});
    chip.addEventListener('pointermove',function(e){if(!timer||e.pointerType==='touch')return;if(Math.hypot(e.clientX-startX,e.clientY-startY)>12)clear()});
    chip.addEventListener('pointerup',clear);chip.addEventListener('pointercancel',clear);chip.addEventListener('pointerleave',clear);
  ...
  function logTaskDrag(status,e){
    ...
    window.__stretchDiagnostics.log('task-drag',{status:status,elapsed:dragDebugStarted?Date.now()-dragDebugStarted:0,moves:dragDebugMoves,active:dragTouchActive,cancelable:!!(e&&e.cancelable),eventType:e&&e.type||'',y:Math.round(dragPointerY),speed:dragScrollSpeed,scroll:Math.round(scroller&&scroller.scrollTop||0),pool:r?{top:Math.round(r.top),bottom:Math.round(r.bottom)}:null,hit:pointInPool(dragPointerY),touchAction:dragHeld?getComputedStyle(dragHeld).touchAction:''});
  }
  ...
  function createTaskGhost(){if(!dragHeld)return;var title=dragHeld.querySelector('.task-title-text'),r=dragHeld.getBoundingClientRect();dragGhost=document.createElement('div');dragGhost.className='task-drag-ghost';dragGhost.textContent=title?title.textContent:'タスク';dragGhost.style.left=Math.max(8,r.left)+'px';dragGhost.style.width=Math.min(r.width,innerWidth-16)+'px';document.body.appendChild(dragGhost);moveTaskGhost()}
  function moveTaskGhost(){if(!dragGhost)return;dragGhost.style.top=clamp(dragPointerY-26,8,innerHeight-62)+'px'}
  ...
  function bindUnifiedTaskDrag(){
    ...
    function activateHold(){if(!dragHeld)return;dragTouchActive=true;dragHeld.classList.remove('swipe-open','swipe-copy-open');dragHeld.classList.add('schedule-dragging','task-dragging');createTaskGhost();updateTaskDropTarget(dragPointerY);updateTaskAutoScroll(dragPointerY);logTaskDrag(dragNativeTouch?'touch-hold-active':'pointer-hold-active');if(navigator.vibrate)navigator.vibrate(25)}
    ...
    window.addEventListener('touchstart',function(e){var handle=e.target&&e.target.closest?e.target.closest('#taskOpenList .task-drag-handle'):null,row=handle&&handle.closest('.task-row:not(.completed)');if(!handle||!row||e.touches.length!==1)return;var t=e.changedTouches&&e.changedTouches[0]||e.touches[0];if(e.cancelable)e.preventDefault();e.stopImmediatePropagation();dragTouchId=t.identifier;startGesture(row,t.clientX,t.clientY,'touch',e);dragTouchId=t.identifier},{passive:false,capture:true});
    window.addEventListener('pointerdown',function(e){if(e.pointerType==='touch')return;var handle=e.target&&e.target.closest?e.target.closest('#taskOpenList .task-drag-handle'):null,row=handle&&handle.closest('.task-row:not(.completed)');if(!handle||!row||(e.button!=null&&e.button!==0))return;e.preventDefault();startGesture(row,e.clientX,e.clientY,'pointer',e);dragPointerId=e.pointerId;handle.setPointerCapture&&handle.setPointerCapture(e.pointerId)},{capture:true});
    ...
    window.addEventListener('touchmove',function(e){if(!dragNativeTouch||!dragHeld||e.touches.length!==1)return;var t=e.touches[0];dragDebugMoves++;dragPointerX=t.clientX;dragPointerY=t.clientY;if(e.cancelable)e.preventDefault();e.stopImmediatePropagation();if(!dragTouchActive){logTaskDrag('touchmove-before-hold',e);return}moveTaskGhost();updateTaskDropTarget(dragPointerY);updateTaskAutoScroll(dragPointerY);logTaskDrag('touchmove-direct',e)},{passive:false,capture:true});
    function complete(cancelled,e,statusPrefix){if(!dragHeld)return;clearTimeout(dragHold);dragHold=null;if(e&&e.cancelable)e.preventDefault();if(e&&e.stopImmediatePropagation)e.stopImmediatePropagation();var id=dragTaskId,targetId=dragTarget&&dragTarget.dataset.id,before=dragBefore,inPool=!cancelled&&pointInPool(dragPointerY),status=cancelled?(statusPrefix||'touchcancel'):inPool?'end-in-pool':targetId?'end-on-row':'end-outside';logTaskDrag(status,e);dragSuppressClickUntil=Date.now()+600;resetTaskGesture();if(cancelled)return;if(inPool)addTask(id,false);else if(targetId)reorderTaskDirect(id,targetId,before)}
    function finishPointer(e){if(!dragHeld||e.pointerId!==dragPointerId)return;if(e.type==='pointercancel'){if(!dragTouchActive){logTaskDrag('cancel-before-hold',e);resetTaskGesture();return}logTaskDrag('pointer-handoff',e);dragPointerId=null;return}if(!dragTouchActive){logTaskDrag('end-before-hold',e);resetTaskGesture();return}if(Number.isFinite(e.clientX)&&Number.isFinite(e.clientY)){dragPointerX=e.clientX;dragPointerY=e.clientY}complete(false,e,'pointerup')}
    window.addEventListener('pointerup',finishPointer,{passive:false,capture:true});window.addEventListener('pointercancel',finishPointer,{passive:false,capture:true});
    window.addEventListener('touchend',function(e){if(!dragHeld||!dragNativeTouch)return;var t=Array.prototype.find.call(e.changedTouches||[],function(x){return x.identifier===dragTouchId})||e.changedTouches&&e.changedTouches[0];if(t){dragPointerX=t.clientX;dragPointerY=t.clientY}if(!dragTouchActive){logTaskDrag('touchend-before-hold',e);resetTaskGesture();return}complete(false,e,'touchend-direct')},{passive:false,capture:true});
    window.addEventListener('touchcancel',function(e){if(!dragHeld||!dragNativeTouch)return;complete(true,e,'touchcancel-direct')},{passive:false,capture:true});
    document.addEventListener('click',function(e){if(Date.now()<dragSuppressClickUntil&&e.target.closest('#taskOpenList .task-row')){e.preventDefault();e.stopImmediatePropagation()}},true)
  }
