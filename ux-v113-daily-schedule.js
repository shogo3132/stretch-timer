(function(){
  ...
  var dragTaskId='',dragTouchActive=false,dragHold=null,dragHeld=null,dragTarget=null,dragBefore=true,dragStartX=0,dragStartY=0,dragPointerX=0,dragPointerY=0,dragPointerId=null,dragGhost=null,dragScrollSpeed=0,dragScrollFrame=null,dragLastScrollAt=0,dragSuppressClickUntil=0,dragDebugMoves=0,dragDebugStarted=0,editorId='',actionTaskId='',clockTimer=null,observer=null;
...
';
  ...
  function bindUnifiedTaskDrag(){
    ...
    window.addEventListener('touchmove',function(e){if(!dragTouchActive||!dragHeld||e.touches.length!==1)return;var t=e.touches[0];dragPointerX=t.clientX;dragPointerY=t.clientY;if(e.cancelable)e.preventDefault();e.stopImmediatePropagation();moveTaskGhost();updateTaskDropTarget(dragPointerY);updateTaskAutoScroll(dragPointerY);logTaskDrag('touchmove-guard',e)},{passive:false,capture:true});
    ...
    window.addEventListener('touchend',function(e){if(!dragHeld||!dragTouchActive)return;var t=e.changedTouches&&e.changedTouches[0];if(t){dragPointerX=t.clientX;dragPointerY=t.clientY}complete(false,e,'touchend')},{passive:false,capture:true});
    window.addEventListener('touchcancel',function(e){if(!dragHeld)return;complete(true,e,'touchcancel')},{passive:false,capture:true});
