...
.daily-event{position:absolute;min-height:22px;border:0;border-left:4px solid var(--event-line);border-radius:9px;background:var(--event-bg);color:var(--event-text);padding:5px 61px 4px 7px;text-align:left;overflow:hidden;box-shadow:0 1px 3px rgba(29,39,48,.08);touch-action:none;cursor:grab}\
...
.daily-event-duplicate{display:grid;place-items:center;width:20px;height:20px;border:1.5px solid var(--event-line);border-radius:6px;background:rgba(255,255,255,.86);color:var(--event-text);padding:0;font-size:15px;font-weight:900;line-height:1;cursor:pointer}\
.daily-event-duplicate:active{background:var(--event-line);color:#fff}.daily-event-duplicate:disabled{cursor:default;opacity:.55}\
...
';
  ...
  function duplicateEntry(entry){
    if(selectedDayOffset<0||!entry||entry.start==null||entry.end==null)return;
    var data=ensureData(false),duration=Math.max(SNAP,entry.end-entry.start),start=entry.end,end=start+duration;
    if(end>DAY_END){confirmSchedule('翌6時までに同じ長さの枠を置けません');return}
    var copy={id:uid2(),taskId:entry.taskId,title:entry.title,color:entry.color,order:data.nextOrder++,start:start,end:end,completedAt:0};
    data.entries.push(copy);saveAndRender();refreshTaskLists();logSchedule('duplicate-saved',{entryId:entry.id,copyId:copy.id,taskId:entry.taskId,day:data.day,start:start,end:end});confirmSchedule('予定を複製しました');
  }
  ...
  function removeTaskFromSelectedDay(taskId){if(selectedDayOffset<0)return false;var data=ensureData(false),before=data.entries.length;data.entries=data.entries.filter(function(entry){return entry.taskId!==taskId});if(data.entries.length===before)return false;saveAndRender();refreshTaskLists();confirmSchedule(dayLabel(selectedDayOffset)+'の予定から外しました');return true}
  ...
  function renderSchedule(){
    ...
    var layout=eventLayout(scheduled);scheduled.forEach(function(entry){var task=taskForEntry(entry),done=!!entry.completedAt;var pos=layout[entry.id]||{col:0,cols:1},event=document.createElement('div');event.className='daily-event'+(done?' completed':'');event.setAttribute('role','button');event.tabIndex=0;event.dataset.entryId=entry.id;event.style.top=((entry.start-range.start)/60*HOUR_PX)+'px';event.style.height=Math.max(22,(entry.end-entry.start)/60*HOUR_PX-2)+'px';event.style.left='calc('+(pos.col/pos.cols*100)+'% + 2px)';event.style.width='calc('+(100/pos.cols)+'% - 4px)';applyPalette(event,entry);event.innerHTML='<span class="daily-event-handle resize-top" data-resize="top"></span><span class="daily-event-title"></span><span class="daily-event-time"></span><span class="daily-event-status"></span><span class="daily-event-handle resize-bottom" data-resize="bottom"></span>';event.querySelector('.daily-event-title').textContent=task.title;event.querySelector('.daily-event-time').textContent=timeText(entry.start)+'〜'+timeText(entry.end);var status=event.querySelector('.daily-event-status');if(task.repeatDaily){var repeat=document.createElement('span');repeat.className='daily-event-repeat';repeat.textContent='↻';repeat.title='毎日タスク';status.appendChild(repeat)}var duplicate=document.createElement('button');duplicate.type='button';duplicate.className='daily-event-duplicate';duplicate.textContent='⧉';duplicate.disabled=selectedDayOffset<0;duplicate.title='同じ予定を複製';duplicate.setAttribute('aria-label','同じ予定を複製');duplicate.onclick=function(e){e.preventDefault();e.stopPropagation();duplicateEntry(entry)};status.appendChild(duplicate);var check=document.createElement('button');check.type='button';check.className='daily-event-check'+(done?' checked':'');check.textContent=done?'✓':'';check.disabled=selectedDayOffset!==0;check.setAttribute('aria-label',done?'今日の完了を取り消す':'今日のタスクを完了');check.onclick=function(e){e.preventDefault();e.stopPropagation();toggleEntryComplete(entry)};status.appendChild(check);event.onclick=function(e){if(e.target.closest('.daily-event-check,.daily-event-duplicate'))return;if(event.dataset.moved==='1'){event.dataset.moved='';return}openEntryTask(entry)};event.onkeydown=function(e){if((e.key==='Enter'||e.key===' ')&&!e.target.closest('.daily-event-check,.daily-event-duplicate')){e.preventDefault();openEntryTask(entry)}};wireEvent(event,entry,range);events.appendChild(event)});
