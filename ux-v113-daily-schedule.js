...
';
  ...
  function bindUnifiedTaskDrag(){
    ...
    window.addEventListener('pointerdown',function(e){var target=e.target&&e.target.closest?e.target:null,row=target&&target.closest('#taskOpenList .task-row:not(.completed)'),handle=target&&target.closest('.task-drag-handle'),blocked=target&&target.closest('button,input,textarea,a');if(!row||(!handle&&blocked)||(e.button!=null&&e.button!==0))return;resetTaskGesture();e.preventDefault();dragHeld=row;dragTaskId=row.dataset.id;dragPointerId=e.pointerId;dragStartX=dragPointerX=e.clientX;dragStartY=dragPointerY=e.clientY;dragDebugMoves=0;dragDebugStarted=Date.now();var captureTarget=handle||row;captureTarget.setPointerCapture&&captureTarget.setPointerCapture(e.pointerId);logTaskDrag('pointerdown',e);dragHold=setTimeout(function(){if(!dragHeld)return;dragTouchActive=true;dragHeld.classList.remove('swipe-open','swipe-copy-open');dragHeld.classList.add('schedule-dragging','task-dragging');createTaskGhost();updateTaskDropTarget(dragPointerY);updateTaskAutoScroll(dragPointerY);logTaskDrag('hold-active');if(navigator.vibrate)navigator.vibrate(25)},TASK_HOLD_MS)},{capture:true});
