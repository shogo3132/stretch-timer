...
';
  ...
  function wireChipReturn(chip,entry){
    ...
    function finish(e){if(e&&e.cancelable)e.preventDefault();var shouldReturn=active&&outside();if(active)logSchedule(shouldReturn?'return-drag-drop':'return-drag-cancel',{entryId:entry.id,taskId:entry.taskId,y:Math.round(currentY)});clear();if(shouldReturn){removeEntry(entry.id);confirmPool('全タスクへ戻しました')}}
