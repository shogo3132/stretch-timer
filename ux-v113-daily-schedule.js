...
';
  ...
  function bindUnifiedTaskDrag(){
    window.addEventListener('touchstart',function(e){if(e.touches.length!==1)return;var row=e.target&&e.target.closest?e.target.closest('#taskOpenList .task-row:not(.completed)'):null;if(!row||e.target.closest('button,input,textarea,select,a'))return;resetTaskGesture();dragHeld=row;dragTaskId=row.dataset.id;dragStartX=e.touches[0].clientX;dragStartY=e.touches[0].clientY;dragPointerY=dragStartY;dragDebugMoves=0;dragDebugStarted=Date.now();logTaskDrag('touchstart',e);dragHold=setTimeout(function(){if(!dragHeld)return;dragTouchActive=true;dragHeld.classList.remove('swipe-open','swipe-copy-open');dragHeld.classList.add('schedule-dragging','task-dragging');updateTaskDropTarget(dragPointerY);updateTaskAutoScroll(dragPointerY);logTaskDrag('hold-active');if(navigator.vibrate)navigator.vibrate(25)},420)},{passive:true,capture:true});
    window.addEventListener('touchmove',function(e){if(!dragHeld||e.touches.length!==1)return;dragDebugMoves++;var p=e.touches[0],dx=p.clientX-dragStartX,dy=p.clientY-dragStartY;dragPointerY=p.clientY;if(!dragTouchActive){logTaskDrag('move-before-hold',e);if(Math.hypot(dx,dy)>16)resetTaskGesture();return}if(e.cancelable)e.preventDefault();e.stopImmediatePropagation();updateTaskDropTarget(dragPointerY);updateTaskAutoScroll(dragPointerY);logTaskDrag('touchmove',e)},{passive:false,capture:true});
    function finish(e){if(!dragHeld)return;clearTimeout(dragHold);dragHold=null;if(!dragTouchActive){logTaskDrag(e&&e.type==='touchcancel'?'cancel-before-hold':'end-before-hold',e);resetTaskGesture();return}if(e&&e.stopImmediatePropagation)e.stopImmediatePropagation();if(e&&e.changedTouches&&e.changedTouches.length)dragPointerY=e.changedTouches[0].clientY;var id=dragTaskId,targetId=dragTarget&&dragTarget.dataset.id,before=dragBefore,inPool=pointInPool(dragPointerY),status=e&&e.type==='touchcancel'?'touchcancel':inPool?'end-in-pool':targetId?'end-on-row':'end-outside';logTaskDrag(status,e);dragSuppressClickUntil=Date.now()+600;resetTaskGesture();if(inPool)addTask(id,false);else if(targetId)reorderTaskDirect(id,targetId,before)}
