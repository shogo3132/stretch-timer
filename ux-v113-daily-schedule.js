...
.schedule-range-groups{display:grid;gap:12px}.schedule-range-groups>.schedule-time-field{margin-bottom:6px}.schedule-range-groups .task-time-fields{display:grid;grid-template-columns:1fr 1fr;gap:9px}.schedule-range-groups .item-time-input-wrap{height:82px;padding:8px}.schedule-range-groups .item-time-input{width:62px}\
...
';
  ...
  function openRangeEditor(){var data=ensureData(false),builder=window.StretchUI&&window.StretchUI.buildTimeNumberInput;if(!builder)return;var overlay=document.createElement('div');overlay.id='scheduleRangeOverlay';overlay.className='schedule-time-overlay';overlay.innerHTML='<div class="schedule-time-panel schedule-range-panel"><div class="schedule-time-title">表示時間</div><div class="schedule-time-sub">今日のスケジュールに表示する基本範囲</div><div class="schedule-range-groups"><div><div class="schedule-time-field">開始</div><div class="task-time-fields" data-range="start"></div></div><div><div class="schedule-time-field">終了</div><div class="task-time-fields" data-range="end"></div></div></div><div id="scheduleRangeError" class="schedule-time-error"></div><div class="schedule-time-actions"><button type="button" class="btn sub" data-act="cancel">キャンセル</button><button type="button" class="btn" data-act="save">設定</button></div></div>';var sh=builder(Math.floor(data.viewStart/60),23,'時'),sm=builder(data.viewStart%60,59,'分'),eh=builder(Math.floor(data.viewEnd/60)%24,24,'時'),em=builder(data.viewEnd===1440?0:data.viewEnd%60,59,'分');overlay.querySelector('[data-range="start"]').append(sh.element,sm.element);overlay.querySelector('[data-range="end"]').append(eh.element,em.element);overlay.onclick=function(e){if(e.target===overlay||e.target.closest('[data-act="cancel"]')){overlay.remove();return}var saveButton=e.target.closest('[data-act="save"]');if(!saveButton)return;var start=sh.value()*60+sm.value(),end=eh.value()*60+em.value(),error=overlay.querySelector('#scheduleRangeError');if(eh.value()===24&&em.value()!==0){error.textContent='24時を指定する場合、分は00にしてください';return}if(end<=start){error.textContent='終了時刻は開始時刻より後にしてください';return}data.viewStart=clamp(snap(start),0,1425);data.viewEnd=clamp(snap(end),data.viewStart+SNAP,1440);if(typeof save==='function')save();overlay.remove();renderSchedule()};document.body.appendChild(overlay);setTimeout(function(){try{sh.input.focus();sh.input.select()}catch(e){}},0)}
