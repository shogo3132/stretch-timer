...
';
  ...
  function cleanEntry(x,index){
    ...
    return {id:String(x.id||uid2()),taskId:String(x.taskId||''),title:String(x.title||''),color:Math.abs(Math.floor(+x.color||0))%COLORS.length,order:Math.max(0,Math.floor(+x.order||index||0)),start:start,end:end};
  }
  function cleanDay(source,key){
    source=source&&typeof source==='object'?source:{};var viewStart=clamp(snap(source.viewStart==null?DEFAULT_VIEW_START:+source.viewStart),0,1425),viewEnd=clamp(snap(source.viewEnd==null?DEFAULT_VIEW_END:+source.viewEnd),15,1440);
  ...
  function cleanSchedule(source){
    ...
    allowed.forEach(function(key){if(!days[key])days[key]=cleanDay({},key)});return {version:2,days:days};
  }
  ...
  function addTask(id,openEditorAfter){
    ...
    if(!existing){var order=data.nextOrder++;existing={id:uid2(),taskId:id,title:task.title,color:order%COLORS.length,order:order,start:null,end:null};data.entries.push(existing);assignMissingTimes(data.entries);saveAndRender();logSchedule('add-saved',{taskId:id,entryId:existing.id,day:data.day,before:before,after:ensureData(false).entries.length,nextOrder:data.nextOrder});confirmPool(dayLabel(selectedDayOffset)+'のタスクに追加しました')}
    else{if(assignMissingTimes(data.entries)&&typeof save==='function')save();renderSchedule();logSchedule('add-existing',{taskId:id,entryId:existing.id,day:data.day,count:data.entries.length});confirmPool('すでに'+dayLabel(selectedDayOffset)+'のタスクに入っています')}
  ...
  function scheduleRange(entries,data){data=data||ensureData(false);var start=data.viewStart,end=data.viewEnd;entries.forEach(function(entry){if(entry.start!=null)start=Math.min(start,entry.start);if(entry.end!=null)end=Math.max(end,entry.end)});return {start:clamp(Math.floor(start/30)*30,0,1410),end:clamp(Math.ceil(end/30)*30,30,1440)}}
  ...
  function ensureUi(){
    ...
    if(!root){root=document.createElement('section');root.id='dailySchedule';root.className='daily-schedule';root.innerHTML='<div class="daily-day-tabs"><button class="daily-day-tab" type="button" data-day-offset="-1">昨日</button><button class="daily-day-tab" type="button" data-day-offset="0">今日</button><button class="daily-day-tab" type="button" data-day-offset="1">明日</button></div><div class="card daily-schedule-card"><div class="daily-schedule-head"><div id="dailyScheduleDate" class="daily-schedule-date"></div><button id="dailyScheduleRange" class="daily-schedule-range" type="button"></button></div><div id="dailyTimelineScroll" class="daily-timeline-scroll"><div id="dailyTimeline" class="daily-timeline"><div id="dailyEvents" class="daily-events"></div><div id="dailyNow" class="daily-now" hidden></div></div></div><div id="dailyTimelineEmpty" class="daily-schedule-empty">全タスクから長押しして追加できます</div></div><div id="dailyPool" class="daily-pool"><div class="daily-pool-head"><span id="dailyPoolTitle">今日のタスク</span><span class="daily-pool-hint">タスクを長押し</span></div><div id="dailyPoolList" class="daily-pool-list"></div></div>';page.insertBefore(root,page.firstChild);root.querySelector('#dailyScheduleRange').onclick=openRangeEditor;root.querySelectorAll('[data-day-offset]').forEach(function(button){button.onclick=function(){switchDay(+button.dataset.dayOffset)}});wireDaySwipe(root)}
    return root;
  }
  ...
  function renderSchedule(){
    ...
    document.getElementById('dailyScheduleDate').textContent=dayLabel(selectedDayOffset)+'：'+formatDate(data.day);document.getElementById('dailyPoolTitle').textContent=dayLabel(selectedDayOffset)+'のタスク';root.querySelectorAll('[data-day-offset]').forEach(function(button){button.classList.toggle('active',+button.dataset.dayOffset===selectedDayOffset)});document.getElementById('dailyScheduleRange').textContent=Math.floor(data.viewStart/60)+'時〜'+Math.ceil(data.viewEnd/60)+'時';var timeline=document.getElementById('dailyTimeline'),events=document.getElementById('dailyEvents'),empty=document.getElementById('dailyTimelineEmpty');timeline.style.height=height+'px';events.innerHTML='';
  ...
  function wireEvent(el,entry,range){el.addEventListener('pointerdown',function(e){if(e.button!=null&&e.button!==0)return;e.preventDefault();var handle=e.target.closest('[data-resize]'),mode=handle&&handle.dataset.resize||'move',pointerId=e.pointerId,startY=e.clientY,initialStart=entry.start,initialEnd=entry.end,moved=false,finished=false;el.setPointerCapture&&el.setPointerCapture(pointerId);el.classList.add('moving');
      function move(ev){if(ev.pointerId!==pointerId)return;var delta=snap((ev.clientY-startY)/HOUR_PX*60),nextStart=initialStart,nextEnd=initialEnd;if(Math.abs(ev.clientY-startY)>4)moved=true;if(mode==='top')nextStart=clamp(initialStart+delta,0,initialEnd-SNAP);else if(mode==='bottom')nextEnd=clamp(initialEnd+delta,initialStart+SNAP,1440);else{var duration=initialEnd-initialStart;nextStart=clamp(initialStart+delta,0,1440-duration);nextEnd=nextStart+duration}entry.start=nextStart;entry.end=nextEnd;el.style.top=((entry.start-range.start)/60*HOUR_PX)+'px';el.style.height=Math.max(22,(entry.end-entry.start)/60*HOUR_PX-2)+'px';el.querySelector('.daily-event-time').textContent=timeText(entry.start)+'〜'+timeText(entry.end);if(ev.cancelable)ev.preventDefault()}
      function finish(ev){if(finished||(ev&&ev.pointerId!=null&&ev.pointerId!==pointerId))return;if(ev&&ev.type==='pointerup')move(ev);finished=true;document.removeEventListener('pointermove',move,true);document.removeEventListener('pointerup',finish,true);document.removeEventListener('pointercancel',finish,true);el.removeEventListener('lostpointercapture',finish);el.classList.remove('moving');if(moved){el.dataset.moved='1';logSchedule('time-drag-saved',{entryId:entry.id,day:selectedDay(),mode:mode,start:entry.start,end:entry.end});saveAndRender()}}
