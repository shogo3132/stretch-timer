(function(){
  ...
  function decorateRoutineCard(el){
    if(!el||el.dataset.v13Ready==='1')return;
    var id=el.dataset.id;if(!id)return;
    el.dataset.v13Ready='1';
    ...
    if(del){
      var fixedDel=del.cloneNode(true);
      del.replaceWith(fixedDel);
    ...
    dup.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();duplicateRoutineById(id)});
    el.appendChild(dup);
    ...
    el.addEventListener('click',function(e){
      ...
      if(el.classList.contains('swipe-open')||el.classList.contains('swipe-copy-open')){
        el.classList.remove('swipe-open','swipe-copy-open');return;
      }
    ...
    el.addEventListener('touchend',function(e){
      ...
      closeRoutineSwipes(el);
      if(dx>0){el.classList.remove('swipe-open');el.classList.add('swipe-copy-open')}
      else{el.classList.remove('swipe-copy-open');el.classList.add('swipe-open')}
    },{passive:true});
  }
  ...
  async function waitForNewWorker(reg){
    if(!reg)return;
    try{await reg.update()}catch(e){}
    var w=reg.installing||reg.waiting;
    if(!w)return;
    await new Promise(function(resolve){
      var timer=setTimeout(resolve,2800);
  ...
  function getStoredState(){return localStorage.getItem('stretchTimer.v2')||''}
  ...
  async function unifiedRefresh(){
    ...
    var stateBefore=getStoredState();
    ...
    try{
      if(typeof hasDropboxAuth==='function'&&hasDropboxAuth()&&typeof syncNow==='function'){
        ...
        var stateAfter=getStoredState();
        var lastSyncAfter=getLastSync();
        syncChanged=(stateBefore!==stateAfter)||(lastSyncAfter!==lastSyncBefore);
      }

      if(appChanged&&'serviceWorker'in navigator){
        try{var reg=await navigator.serviceWorker.getRegistration();await waitForNewWorker(reg)}catch(e){console.error(e)}
      }
      ...
      if(appChanged){
        ...
        if('caches'in window){
          try{var keys=await caches.keys();await Promise.all(keys.filter(function(k){return k.indexOf('stretch-timer-')===0&&k!==remoteBuild}).map(function(k){return caches.delete(k)}))}catch(e){}
        }
...
';
  ...
  document.addEventListener('touchstart',function(e){
    if(PULL_SCREENS.indexOf(activeScreenId())<0||e.touches.length!==1)return;
  ...
  document.addEventListener('touchend',function(e){
    ...
    var ready=pullReady;
    pullActive=false;pullReady=false;
    indicator.style.transform='translate(-50%,-70px)';
    indicator.classList.remove('visible','ready');
    if(ready){if(e.cancelable)e.preventDefault();unifiedRefresh()}
  },{passive:false,capture:true});
