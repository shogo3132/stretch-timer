...
';
  ...
  function renderDetail(){ensureScreen();var m=getMenu();if(!m)return;closePop();document.getElementById('detailRoutineName').textContent=m.name||'ルーティン';var y=viewDate.getFullYear(),mo=viewDate.getMonth(),daysInMonth=new Date(y,mo+1,0).getDate(),map=monthMap(m,y,mo),today=new Date();var activeDays=Object.keys(map).length,pct=daysInMonth?activeDays/daysInMonth:0,percent=Math.round(pct*100);document.getElementById('detailRing').style.setProperty('--p',(pct*360)+'deg');document.getElementById('detailRing').title=percent+'%';var rv=document.getElementById('detailRingValue');if(rv)rv.textContent=percent+'%';document.getElementById('detailMonthTitle').textContent=y+'年 '+(mo+1)+'月';var box=document.getElementById('detailDays');box.innerHTML='';var first=new Date(y,mo,1).getDay();for(var i=0;i<first;i++){var blank=document.createElement('div');blank.className='detail-day blank';box.appendChild(blank)}for(var day=1;day<=daysInMonth;day++){var k=y+'-'+String(mo+1).padStart(2,'0')+'-'+String(day).padStart(2,'0'),arr=map[k]||[],isToday=y===today.getFullYear()&&mo===today.getMonth()&&day===today.getDate();var b=document.createElement('button');b.type='button';b.className='detail-day'+(arr.length>=2?' done2':arr.length===1?' done1':'')+(arr.length?' has-log':'')+(isToday?' today':'');b.textContent=day;if(arr.length)(function(btn,t){btn.onclick=function(e){if(Date.now()<suppressDayTapUntil){e.preventDefault();e.stopPropagation();return}e.stopPropagation();showPop(btn,t.slice())}})(b,arr);box.appendChild(b)}document.getElementById('detailCompleteCount').textContent='完走 '+logs(m).length+'回'}
