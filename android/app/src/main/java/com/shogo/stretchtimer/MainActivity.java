...
public class MainActivity extends Activity {
    ...
    void buildShell(){
        ...
        LinearLayout bar=row();bar.setPadding(dp(14),dp(10),dp(14),dp(10));bar.setBackgroundColor(Color.WHITE);
        back=btn("←");back.setOnClickListener(v->goBack());LinearLayout.LayoutParams bp=new LinearLayout.LayoutParams(dp(58),dp(48));bp.setMargins(0,0,dp(10),0);bar.addView(back,bp);
    ...
    void showHome(){
        ...
        for(Menu m:menus){LinearLayout card=new LinearLayout(this);card.setOrientation(LinearLayout.VERTICAL);card.setPadding(dp(18),dp(18),dp(18),dp(18));card.setBackground(bg(Color.WHITE,20));LinearLayout top=row();TextView t=tv(m.name,21);t.setTypeface(Typeface.DEFAULT_BOLD);top.addView(t,new LinearLayout.LayoutParams(0,-2,1));Button ed=btn("設定");ed.setOnClickListener(v->showMenu(m));top.addView(ed,new LinearLayout.LayoutParams(dp(82),-2));card.addView(top);TextView meta=tv(m.items.size()+"項目  ・  休憩 "+m.rest+"秒",14);meta.setTextColor(Color.rgb(110,118,128));meta.setPadding(0,dp(2),0,dp(12));card.addView(meta);Button st=btn("▶  このメニューを開始");st.setTextSize(16);st.setTextColor(Color.WHITE);st.setBackground(bg(Color.rgb(39,174,139),16));st.setOnClickListener(v->startTimer(m));card.addView(st,new LinearLayout.LayoutParams(-1,dp(52)));LinearLayout.LayoutParams cp=new LinearLayout.LayoutParams(-1,-2);cp.setMargins(0,0,0,dp(18));content.addView(card,cp);}
    }
    ...
    void showItem(Item x){
        ...
        add(label("写真"));ImageView preview=new ImageView(this);preview.setScaleType(ImageView.ScaleType.CENTER_CROP);setImage(preview,x.photo);LinearLayout.LayoutParams pp=new LinearLayout.LayoutParams(-1,dp(230));pp.setMargins(0,0,0,dp(14));content.addView(preview,pp);
        Runnable persist=()->persistFields(n,sec,desc);
        LinearLayout pr=row();Button pic=btn("スマホ内から選ぶ"),cam=btn("カメラで撮影");pic.setOnClickListener(v->{persist.run();Intent i=new Intent(Intent.ACTION_OPEN_DOCUMENT);i.setType("image/*");i.addCategory(Intent.CATEGORY_OPENABLE);i.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION|Intent.FLAG_GRANT_PERSISTABLE_URI_PERMISSION);startActivityForResult(i,PICK_IMAGE);});cam.setOnClickListener(v->{persist.run();if(Build.VERSION.SDK_INT>=23&&checkSelfPermission(android.Manifest.permission.CAMERA)!=PackageManager.PERMISSION_GRANTED)requestPermissions(new String[]{android.Manifest.permission.CAMERA},REQ_CAMERA);else launchCamera();});rowGap(pic,cam,pr);add(pr);addGap(14);
        LinearLayout mv=row();Button up=btn("↑ 上へ"),dn=btn("↓ 下へ");up.setOnClickListener(v->{persist.run();moveItem(-1);});dn.setOnClickListener(v->{persist.run();moveItem(1);});rowGap(up,dn,mv);add(mv);addGap(14);
