...
public class MainActivity extends Activity {
    ...
    void showHome(){ currentMenu=null;currentItem=null;clear("ストレッチ",false);action.setVisibility(View.VISIBLE);action.setText("＋ メニュー");action.setOnClickListener(v->{Menu m=new Menu();m.id=id();m.name="新しいメニュー";menus.add(m);save();showMenu(m);});
        TextView intro=tv("運動メニュー",28);intro.setTypeface(Typeface.DEFAULT_BOLD);add(intro);add(tv("メニューを選ぶと、登録した項目が設定時間ごとに自動で進みます。",15));
        if(menus.isEmpty()){ TextView empty=tv("まだメニューがありません。\n右上の「＋ メニュー」から作成してください。",17);empty.setGravity(Gravity.CENTER);empty.setPadding(0,dp(60),0,dp(60));add(empty); }
        for(Menu m:menus){ LinearLayout card=new LinearLayout(this);card.setOrientation(LinearLayout.VERTICAL);card.setPadding(dp(18),dp(16),dp(18),dp(16));card.setBackground(bg(Color.WHITE,18)); TextView t=tv(m.name,22);t.setTypeface(Typeface.DEFAULT_BOLD);card.addView(t);card.addView(tv(m.items.size()+"項目  ・  休憩 "+m.rest+"秒",14));LinearLayout r=row();Button st=btn("▶ 開始"),ed=btn("設定");st.setOnClickListener(v->startTimer(m));ed.setOnClickListener(v->showMenu(m));r.addView(st,new LinearLayout.LayoutParams(0,-2,1));r.addView(ed,new LinearLayout.LayoutParams(0,-2,1));card.addView(r);LinearLayout.LayoutParams cp=new LinearLayout.LayoutParams(-1,-2);cp.setMargins(0,0,0,dp(14));content.addView(card,cp);}
    }
    ...
    void advance(boolean skip){if(!restPhase&&!skip&&currentMenu.rest>0&&timerIndex<currentMenu.items.size()-1){restPhase=true;remainingMs=currentMenu.rest*1000L;totalPhaseMs=remainingMs;showTimer();runTimer();return;}if(restPhase)timerIndex++;else if(skip)timerIndex++;else timerIndex++;if(timerIndex>=currentMenu.items.size()){showComplete();return;}restPhase=false;remainingMs=currentMenu.items.get(timerIndex).seconds*1000L;totalPhaseMs=remainingMs;showTimer();runTimer();}
    ...
    void goBack(){stopTimer();getWindow().clearFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);if(currentItem!=null){currentItem=null;showMenu(currentMenu);}else if(currentMenu!=null){Menu m=currentMenu;currentMenu=null;showHome();}else showHome();}
