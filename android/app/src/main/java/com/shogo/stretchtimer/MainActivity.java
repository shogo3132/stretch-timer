...
public class MainActivity extends Activity {
    ...
    static class Menu { String id,name,desc=""; int rest=15; ArrayList<Item> items=new ArrayList<>(); }
    ...
    int totalSeconds(Menu m){int total=0;for(Item x:m.items)total+=Math.max(1,x.seconds);if(m.items.size()>1)total+=Math.max(0,m.rest)*(m.items.size()-1);return total;}
    String durationText(Menu m){int s=totalSeconds(m);int min=s/60,sec=s%60;return min>0?(sec>0?min+"分"+sec+"秒":min+"分"):sec+"秒";}
    ...
    void enableMenuDrag(View card,Menu target){
        card.setOnLongClickListener(v->{ClipData data=ClipData.newPlainText("menu",target.id);v.startDragAndDrop(data,new View.DragShadowBuilder(v),null,0);v.performHapticFeedback(HapticFeedbackConstants.LONG_PRESS);return true;});
        card.setOnDragListener((v,e)->{if(e.getAction()==DragEvent.ACTION_DRAG_ENTERED){v.setAlpha(.72f);return true;}if(e.getAction()==DragEvent.ACTION_DRAG_EXITED){v.setAlpha(1f);return true;}if(e.getAction()==DragEvent.ACTION_DROP){v.setAlpha(1f);String sid=e.getClipData().getItemAt(0).getText().toString();Menu src=null;for(Menu mm:menus)if(mm.id.equals(sid))src=mm;if(src!=null&&src!=target){int from=menus.indexOf(src),to=menus.indexOf(target);menus.remove(from);if(from<to)to--;menus.add(Math.max(0,Math.min(to,menus.size())),src);save();showHome();}return true;}if(e.getAction()==DragEvent.ACTION_DRAG_ENDED){v.setAlpha(1f);return true;}return true;});
    }
    void enableItemDrag(View card,Menu menu,Item target){
        card.setOnLongClickListener(v->{ClipData data=ClipData.newPlainText("item",target.id);v.startDragAndDrop(data,new View.DragShadowBuilder(v),null,0);v.performHapticFeedback(HapticFeedbackConstants.LONG_PRESS);return true;});
    ...
    void showMenu(Menu m){
        ...
        add(label("項目間の休憩時間（秒）"));EditText rest=field("例：15");rest.setInputType(2);rest.setText(""+m.rest);add(rest);
        TextView duration=tv("合計時間  "+durationText(m),14);duration.setTextColor(Color.rgb(110,118,128));duration.setPadding(dp(2),dp(8),0,dp(14));add(duration);
        Runnable saveMenu=()->{m.name=name.getText().toString().trim().isEmpty()?"名称未設定":name.getText().toString().trim();m.desc=desc.getText().toString();try{m.rest=Math.max(0,Integer.parseInt(rest.getText().toString()));}catch(Exception e){}save();};name.setOnFocusChangeListener((v,f)->{if(!f)saveMenu.run();});desc.setOnFocusChangeListener((v,f)->{if(!f)saveMenu.run();});rest.setOnFocusChangeListener((v,f)->{if(!f){saveMenu.run();duration.setText("合計時間  "+durationText(m));}});
        ...
        TextView ih=tv("項目",24);ih.setTypeface(Typeface.DEFAULT_BOLD);ih.setPadding(0,0,0,dp(4));add(ih);TextView dragTip=tv("長押しして並び替え",13);dragTip.setTextColor(Color.rgb(130,137,146));dragTip.setPadding(0,0,0,dp(12));add(dragTip);
        if(m.items.isEmpty()){TextView e=tv("項目がありません。例：屈伸、アキレス腱伸ばし など",16);e.setTextColor(Color.rgb(110,118,128));addM(e,0,12);}
        for(int i=0;i<m.items.size();i++){Item x=m.items.get(i);LinearLayout ir=row();ir.setPadding(dp(12),dp(12),dp(10),dp(12));ir.setBackground(bg(Color.WHITE,16));ImageView im=new ImageView(this);im.setScaleType(ImageView.ScaleType.CENTER_CROP);roundImage(im,14);setImage(im,x.photo);LinearLayout.LayoutParams ip=new LinearLayout.LayoutParams(dp(78),dp(78));ip.setMargins(0,0,dp(14),0);ir.addView(im,ip);TextView tx=tv((i+1)+".  "+x.name+"\n"+x.seconds+"秒",17);tx.setLineSpacing(dp(2),1f);ir.addView(tx,new LinearLayout.LayoutParams(0,-2,1));Button e=btn("編集");LinearLayout.LayoutParams ep=new LinearLayout.LayoutParams(dp(84),-2);ep.setMargins(dp(12),0,0,0);e.setOnClickListener(v->showItem(x));ir.addView(e,ep);enableItemDrag(ir,m,x);LinearLayout.LayoutParams rp=new LinearLayout.LayoutParams(-1,-2);rp.setMargins(0,0,0,dp(12));content.addView(ir,rp);}
        Button dup=btn("このメニューを複製");dup.setOnClickListener(v->{saveMenu.run();Menu c=duplicateMenu(m);menus.add(menus.indexOf(m)+1,c);save();showMenu(c);});addM(dup,8,10);
    ...
    Menu duplicateMenu(Menu m){Menu c=new Menu();c.id=id();c.name=m.name+" コピー";c.desc=m.desc;c.rest=m.rest;for(Item x:m.items){Item y=new Item();y.id=id();y.name=x.name;y.desc=x.desc;y.photo=x.photo;y.seconds=x.seconds;c.items.add(y);}return c;}
    ...
    void showItem(Item x){
        ...
        TextView order=tv("並び順はメニュー画面で項目を長押しして変更できます。",13);order.setTextColor(Color.rgb(120,128,138));order.setPadding(dp(2),0,0,dp(16));add(order);
    ...
    void startTimer(Menu m){if(m.items.isEmpty()){Toast.makeText(this,"このメニューには項目がありません",Toast.LENGTH_SHORT).show();return;}currentMenu=m;currentItem=null;timerIndex=0;restPhase=false;paused=false;showPreStart();}
    void showPreStart(){getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);root.setBackgroundColor(Color.rgb(17,22,30));scroll.setBackgroundColor(Color.rgb(17,22,30));topBar.setBackgroundColor(Color.rgb(17,22,30));barTitle.setText(currentMenu.name);barTitle.setTextColor(Color.WHITE);back.setVisibility(View.VISIBLE);back.setTextColor(Color.WHITE);back.setBackground(bg(Color.rgb(31,38,48),15));action.setVisibility(View.GONE);content.removeAllViews();content.setPadding(dp(24),dp(80),dp(24),dp(34));TextView label=tv("開始まで",17);label.setTextColor(Color.rgb(172,181,195));label.setGravity(Gravity.CENTER);add(label);TextView num=tv("3",92);num.setTag("precount");num.setTextColor(Color.WHITE);num.setGravity(Gravity.CENTER);num.setTypeface(Typeface.DEFAULT_BOLD);addM(num,24,14);TextView first=tv(currentMenu.items.get(0).name,24);first.setTextColor(Color.rgb(87,220,187));first.setGravity(Gravity.CENTER);first.setTypeface(Typeface.DEFAULT_BOLD);add(first);timer=new CountDownTimer(3200,1000){int n=3;public void onTick(long ms){TextView v=content.findViewWithTag("precount");if(v!=null){v.setText(""+n);tone.startTone(ToneGenerator.TONE_PROP_BEEP,80);}n--;}public void onFinish(){remainingMs=currentMenu.items.get(0).seconds*1000L;totalPhaseMs=remainingMs;showTimer();runTimer();}};timer.start();}
