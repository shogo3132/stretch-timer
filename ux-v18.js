...
';
  ...
  function addItem(){var m=currentMenu();if(!m||!Array.isArray(m.items))return;var id=uid2();m.items.push({id:id,name:'新しいステップ',seconds:30,desc:'',photo:''});if(typeof save==='function')save();if(typeof renderItems==='function')renderItems();if(typeof updateDuration==='function')updateDuration();reveal('.item',id)}
  ...
  if(window.StretchUI&&StretchUI.registerReorder){
    ...
    StretchUI.registerReorder({key:'item-cards',selector:'.item',ignore:'button,input,textarea,select,a',label:function(card){var title=card.querySelector('.item-title');return title?title.textContent:'ステップ'},onReorder:function(move){reorderDirect('item',move.id,move.targetId,move.before);settleCard('.item',move.id)}});
  }
