(function(){
  var CURRENT_BUILD='stretch-timer-v30';
  ...
  function ensureRefreshButton(){var settings=document.getElementById('topAction');if(!settings)return;var btn=document.getElementById('refreshBtn');if(!btn){btn=document.createElement('button');btn.id='refreshBtn';btn.className='btn sub icon';btn.textContent='↻';btn.setAttribute('aria-label','更新');btn.title='更新';settings.insertAdjacentElement('afterend',btn)}btn.style.display=activeScreenId()==='home'?'inline-block':'none';btn.onclick=unifiedRefresh}
  ...
  function decorateCard(oldEl,type){
    ...
    el.addEventListener('click',function(e){if(e.target.closest('button')||Date.now()<suppressClickUntil)return;if(el.classList.contains('swipe-open')||el.classList.contains('swipe-copy-open')){closeCard(el);return}e.preventDefault();e.stopPropagation()});
    return el;
  }
