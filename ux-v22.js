(function(){
  ...
  function ensureRefreshButton(){var settings=document.getElementById('topAction');if(!settings)return;var btn=document.getElementById('refreshBtn');if(!btn){btn=document.createElement('button');btn.id='refreshBtn';btn.className='btn sub icon';btn.textContent='↻';btn.setAttribute('aria-label','更新');btn.title='更新';settings.insertAdjacentElement('afterend',btn)}var active=activeScreenId(),top=['home','tasks','recipes','settings'].indexOf(active)>=0;btn.style.display=top?'inline-block':'none';btn.onclick=unifiedRefresh}
