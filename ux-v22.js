(function(){
  var CURRENT_BUILD='stretch-timer-v22';
  ...
  function ensureRefreshButton(){
    ...
    if(!btn){
      btn=document.createElement('button');btn.id='refreshBtn';btn.className='btn sub icon';btn.textContent='↻';
      btn.setAttribute('aria-label','更新');btn.title='更新';settings.insertAdjacentElement('afterend',btn);
    }
  ...
  function decorateCard(oldEl,type){
    ...
    el.addEventListener('touchmove',function(e){
      if(e.touches.length!==1)return;var dx=e.touches[0].clientX-x0,dy=e.touches[0].clientY-y0;
      if(Math.abs(dx)>22&&Math.abs(dx)>Math.abs(dy)*1.25)moved=true;
    },{passive:true});
    el.addEventListener('touchend',function(e){
      if(!moved||!e.changedTouches.length)return;var dx=e.changedTouches[0].clientX-x0,dy=e.changedTouches[0].clientY-y0;
