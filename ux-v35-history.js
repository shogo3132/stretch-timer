(function(){
  if(window.__executionTimeGraphV35)return;
  window.__executionTimeGraphV35=true;

  var NS='http://www.w3.org/2000/svg';
  var style=document.createElement('style');
  style.setAttribute('data-execution-time-graph-v35','');
  style.textContent='\
.detail-time-card{background:#fff;border-radius:20px;padding:15px 14px 12px;box-shadow:0 1px 2px rgba(0,0,0,.04);overflow:hidden}\
...
.detail-time-note{font-size:11px;color:#8a929c;white-space:nowrap}\
.detail-time-empty{height:138px;display:grid;place-items:center;color:#9aa1aa;font-size:13px}\
.detail-time-chart{display:block;width:100%;height:auto;overflow:visible}\
.detail-time-grid{stroke:#e8ecef;stroke-width:1}\
.detail-time-axis{fill:#8a929c;font-size:10px;font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}\
.detail-time-line{fill:none;stroke:#27ae8b;stroke-width:2.6;stroke-linecap:round;stroke-linejoin:round}\
.detail-time-dot{fill:#fff;stroke:#27ae8b;stroke-width:2.2}\
';
  ...
  function ensureCard(){
    ...
    card.innerHTML='<div class="detail-time-head"><div class="detail-time-title">実行時間</div><div class="detail-time-note">完走した時刻</div></div><div id="detailTimeGraph"></div>';
  ...
  function render(){
    ...
    var pts=logs.map(function(ts){var d=new Date(ts);return {ts:ts,day:d.getDate(),serial:dayNumber(ts),hour:hourValue(ts)}}),b=bounds(pts);
    var W=640,H=220,L=48,R=12,T=12,B=30,pw=W-L-R,ph=H-T-B;
    ...
    for(var i=0;i<=tickCount;i++){
      ...
      svg.appendChild(svgEl('text',{x:L-8,y:yy+3,'text-anchor':'end',class:'detail-time-axis'},String(Math.round(hv))+':00'));
    }

    var xticks=[1,5,10,15,20,25,daysInMonth].filter(function(v,idx,a){return v<=daysInMonth&&a.indexOf(v)===idx});
    xticks.forEach(function(day){svg.appendChild(svgEl('text',{x:x(day),y:H-7,'text-anchor':'middle',class:'detail-time-axis'},String(day)))});
    ...
    pts.forEach(function(p){
      var c=svgEl('circle',{cx:x(p.day),cy:y(p.hour),r:4.2,class:'detail-time-dot'});
      c.appendChild(svgEl('title',{},p.day+'日 '+fmtTime(p.ts)));
      svg.appendChild(c);
    });
    box.appendChild(svg);
  }
