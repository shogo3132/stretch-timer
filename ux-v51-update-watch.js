(function(){
  ...
  var CURRENT_BUILD='stretch-timer-v119';
  var POLL_MS=10000;
  ...
  style.setAttribute('data-update-watch-v96','');
  style.textContent='\
...
';
  ...
  function hideBadge(){clearPendingUpdate();foundBuild='';var badge=ensureBadge();if(badge){badge.classList.remove('show','updating');badge.onclick=applyUpdate}setBadgeText('更新あり')}
  ...
  async function check(){if(checking||updating||document.hidden)return;checking=true;try{var build=await remoteBuild(),current=await installedBuild();if(build&&build!==current)showBadge(build);else if(build&&build===current)hideBadge()}finally{checking=false}}
  ...
  async function installedBuild(){return await controllerBuild()||CURRENT_BUILD}
